...
class AddRecommendationsFragment : BaseFragment(), View.OnClickListener {
    ...
    override fun setLanguageData() {
        mBinding.apply {
            ...
            actionbar.title = lang?.emrScreens?.addRecommendation.getSafe()
    ...
    override fun setListeners() {
        mBinding.apply {
            ...
            bCreate.setOnClickListener(this@AddRecommendationsFragment)
        }
    }
...
//                                loading = false
                                ...
                                if(isLastPage.not()){
                                    getMedicalHealthCares(PageRequest(page = emrViewModel.emrFilterRequest.page))
                                    loading = true
                                }
                            }
                        }
    ...
    private fun showAddEmrBindingDialog(item: GenericItem? = null) {
        builder = AlertDialog.Builder(requireActivity(), R.style.AlertDialogTheme).apply {
            ...
            setTitle(mBinding.lang?.emrScreens?.addRecommendation.getSafe())
        ...
        builder.setOnShowListener{
            ...
            dialogSaveButton.setOnClickListener {
                val type = metaData?.emrTypes?.find { it.genericItemId == Enums.EMRTypesMeta.MEDICAL_HEALTHCARE.key }?.genericItemId
    ...
    private fun getMedicalHealthCares(pageRequest: PageRequest) {
        if (isOnline(requireActivity())) {
            emrViewModel.getMedicalHealthCares(pageRequest).observe(this){
                when (it) {
                    is ResponseResult.Success -> {
                        ...
                        response.data?.let { res ->
                            ...
                            tempList.addAll(res.medicalHealthCares?.data as ArrayList<GenericItem>?  ?: arrayListOf())
