...
class WalkInPharmacyMapViewFragment : BaseFragment(), GoogleMap.OnCameraIdleListener {
        ...
        if (System.currentTimeMillis() > lastTextEdit + delay - 500) {
            ...
            if (activity != null && isAdded) {
                lifecycleScope.launch {
                    mMap.clear()
                    getWalkInPharmacyList(displayName = search)
                }
            }
        }
    }
    ...
    private fun setObserver() {
        walkInViewModel.walkInPharmacyMapLiveData.observe(this) { walkInPharmaResponse ->
            lifecycleScope.launch {
                showLoader()
                setMap()
                delay(900)
                if(::mMap.isInitialized) {
                    hideLoader()
                    walkInPharmaResponse?.forEachIndexed{ index, walkInItem ->
                        ...
                        mMap.setOnMarkerClickListener {
                            ...
                            showSelectPharmacyDialog(walkInPharmaResponse[pos])
                            false
                        }
                    }
    ...
    private fun showSelectPharmacyDialog(walkInItem: WalkInItemResponse) {
        builder = AlertDialog.Builder(requireActivity(), R.style.AlertDialogTheme).apply {
            ...
            setTitle(mBinding.lang?.globalString?.selectPharmacy.getSafe())
            setPositiveButton(mBinding.lang?.globalString?.select.getSafe()) { _, _ ->
        ...
        builder.setOnShowListener{
            ...
            dialogSaveButton.setOnClickListener {
                ...
                walkInViewModel.pharmacyId = walkInItem.id
                walkInViewModel.walkInPharmacyName = walkInItem.displayName
    ...
    private fun getWalkInPharmacyList(displayName: String? = null, page: Int? = null) {
                ...
                when(it) {
                    is ResponseResult.Success -> {
                        ...
                        (response.data as WalkInPharmacyListResponse).let { pharmaciesList ->
                            walkInViewModel.walkInPharmacyMapLiveData.postValue(
                                pharmaciesList.walkInPharmacies?.walkInList.getSafe()
                            )
                            if ((pharmaciesList.walkInPharmacies?.walkInList.getSafe()).isEmpty())
                                showToast(mBinding.lang?.globalString?.noWalkInPharmacy.getSafe())
                        }
                    }
